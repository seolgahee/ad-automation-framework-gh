...
export class CreativePipeline {
  ...
  _initCreativeTable() {
    ...
    `);
    // Migrate existing DB (adds P.D.A columns if they don't exist yet)
    for (const col of ['persona_tag', 'desire_tag', 'awareness_stage']) {
      try { db.exec(`ALTER TABLE creatives ADD COLUMN ${col} TEXT`); } catch {}
    }
    db.exec(`
   ...
   */
  async assembleCreative({ templateId, variables, platform, mediaPath, landingUrl, abGroup, persona_tag, desire_tag, awareness_stage }) {
    ...
    db.prepare(`
      INSERT INTO creatives (id, platform, name, type, headline, description, body_text,
        cta, media_hash, landing_url, template_id, ab_group, persona_tag, desire_tag, awareness_stage, metadata_json)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
    ...
    logger.info('Creative assembled', { creativeId, platform, templateId, abGroup, persona_tag, desire_tag, awareness_stage });
  ...
  /**
   * Check P.D.A semantic diversity for a set of creatives
   * Warns if combinations are duplicated (Andromeda self-competition risk)
   */
  checkSemanticDiversity(creatives) {
    const tagged = creatives.filter(c => c.persona_tag && c.desire_tag && c.awareness_stage);
    if (tagged.length === 0) return { isDiverse: null, message: 'P.D.A 태그 없음 — 다양성 분석 불가' };

    const combos = tagged.map(c => `${c.persona_tag}|${c.desire_tag}|${c.awareness_stage}`);
    const uniqueCombos = new Set(combos);
    const duplicated = combos.filter((c, i) => combos.indexOf(c) !== i);

    return {
      isDiverse: uniqueCombos.size === combos.length,
      totalCreatives: tagged.length,
      uniqueCombinations: uniqueCombos.size,
      coverageScore: Math.round((uniqueCombos.size / combos.length) * 100),
      duplicatedCombos: [...new Set(duplicated)],
      warning: duplicated.length > 0
        ? `${duplicated.length}개 소재가 동일한 P.D.A 조합 → Andromeda Self-competition 위험`
        : null,
    };
  }
